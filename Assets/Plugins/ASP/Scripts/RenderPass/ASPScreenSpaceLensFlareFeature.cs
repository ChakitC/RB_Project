...
namespace ASP
{
    ...
    {
         static readonly int s_lensFlareScreenSpaceBloomMipTexture =
 Shader.PropertyToID("_LensFlareScreenSpaceBloomMipTexture");
         static readonly int s_lensFlareScreenSpaceResultTexture =
 Shader.PropertyToID("_LensFlareScreenSpaceResultTexture");
         static readonly int s_lensFlareScreenSpaceSpectralLut = Shader.PropertyToID("_LensFlareScreenSpaceSpectralLut");
         static readonly int s_lensFlareScreenSpaceStreakTex = Shader.PropertyToID("_LensFlareScreenSpaceStreakTex");
         static readonly int s_lensFlareScreenSpaceMipLevel = Shader.PropertyToID("_LensFlareScreenSpaceMipLevel");
         static readonly int s_lensFlareScreenSpaceTintColor = Shader.PropertyToID("_LensFlareScreenSpaceTintColor");
         static readonly int s_lensFlareScreenSpaceParams1 = Shader.PropertyToID("_LensFlareScreenSpaceParams1");
         static readonly int s_lensFlareScreenSpaceParams2 = Shader.PropertyToID("_LensFlareScreenSpaceParams2");
         static readonly int s_lensFlareScreenSpaceParams3 = Shader.PropertyToID("_LensFlareScreenSpaceParams3");
         static readonly int s_lensFlareScreenSpaceParams4 = Shader.PropertyToID("_LensFlareScreenSpaceParams4");
         static readonly int s_lensFlareScreenSpaceParams5 = Shader.PropertyToID("_LensFlareScreenSpaceParams5");
         static readonly int s_bloomTextureId = Shader.PropertyToID("_Bloom_Texture");
        
        [FormerlySerializedAs("injectionPoint")]
        public RenderPassEvent InjectionPoint = RenderPassEvent.AfterRenderingPostProcessing;
        private Material _material;
        private ASPScreenSpaceLensFlarePass _aspScreenSpaceLensFlarePass;

        /// <inheritdoc/>
        public override void Create()
        {
            _aspScreenSpaceLensFlarePass = new ASPScreenSpaceLensFlarePass(name);
        }
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            ...
            _aspScreenSpaceLensFlarePass.Setup(ref renderingData);

            _aspScreenSpaceLensFlarePass.renderPassEvent = (RenderPassEvent)InjectionPoint;
            _aspScreenSpaceLensFlarePass.ConfigureInput(ScriptableRenderPassInput.None);
            _aspScreenSpaceLensFlarePass.SetupMembers(_material);

            renderer.EnqueuePass(_aspScreenSpaceLensFlarePass);
        }

        /// <inheritdoc/>
        protected override void Dispose(bool disposing)
        {
            _aspScreenSpaceLensFlarePass.Dispose();
        }

        public class ASPScreenSpaceLensFlarePass : ScriptableRenderPass
        {
            private Material _lensFlareSSMaterial;
            private RTHandle _copiedColor;
            private RenderTargetHandle _streakTmpTexture;
            private RenderTargetHandle _streakTmpTexture2;
            private RenderTargetHandle _screenSpaceLensFlareResult;
            private ASP.ASPScreenSpaceLensFlare _lensFlareAspScreenSpace;
            private int _writeToBloomPass;
            public ASPScreenSpaceLensFlarePass(string passName)
            {
                profilingSampler = new ProfilingSampler(passName);
                _streakTmpTexture.Init("_StreakTmpTexture");
                _streakTmpTexture2.Init("_StreakTmpTexture2");
                _screenSpaceLensFlareResult.Init("_LensFlareScreenSpaceResultTexture");
            }

            public void SetupMembers(Material material)
            {
                _lensFlareSSMaterial = material;
                _writeToBloomPass = _lensFlareSSMaterial.FindPass("LensFlareScreenSpace Write to BloomTexture");
            }

            public void Setup(ref RenderingData renderingData)
            {
                if (_lensFlareAspScreenSpace == null)
                {
                    _lensFlareAspScreenSpace =
 ...
 RTHandles.Alloc(desc.width, desc.height, 1, DepthBits.None, name:"_CameraColorTexture");
            ...
            {
                cmd.ReleaseTemporaryRT(_streakTmpTexture.id);
                cmd.ReleaseTemporaryRT(_streakTmpTexture2.id);
                cmd.ReleaseTemporaryRT(_screenSpaceLensFlareResult.id);
            }
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                using (new ProfilingScope(cmd, new ProfilingSampler("Post Process Screen Space Lens Flare")))
                {
                    ...
                    _lensFlareSSMaterial.SetVector("_BlitScaleBias", new Vector4(1,1,0,0));
                    UnityEngine.Rendering.CoreUtils.SetRenderTarget(cmd, _copiedColor);
                    ConfigureClear(ClearFlag.Color, Color.black);
                    DrawTriangle(cmd, _lensFlareSSMaterial, _writeToBloomPass);
                    
                    CoreUtils.SetRenderTarget(cmd, cameraData.renderer.cameraColorTarget);
                    Blit(cmd, _copiedColor, cameraData.renderer.cameraColorTarget);
                }
            ...
            { 
                int ratio = (int)_lensFlareAspScreenSpace.resolution.value;
                ...
                if (_lensFlareAspScreenSpace.IsStreaksActive())
                {
                    cmd.GetTemporaryRT(_streakTmpTexture.id, desc, FilterMode.Bilinear);
                    cmd.GetTemporaryRT(_streakTmpTexture2.id, desc, FilterMode.Bilinear);
                ...
                cmd.GetTemporaryRT(_screenSpaceLensFlareResult.id, desc, FilterMode.Bilinear);
              ...
              /*  RenderingUtils.ReAllocateIfNeeded(ref m_ScreenSpaceLensFlareResult, desc, FilterMode.Bilinear,
                ...
                DoLensFlareScreenSpaceCommon(
                    _lensFlareSSMaterial,
                    camera,
                    (float)m_Descriptor.width,
                    (float)m_Descriptor.height,
                    _lensFlareAspScreenSpace.tintColor.value,
                    ...
                    _streakTmpTexture2,
                    new Vector4(
                        _lensFlareAspScreenSpace.intensity.value,
                        _lensFlareAspScreenSpace.firstFlareIntensity.value,
                        _lensFlareAspScreenSpace.secondaryFlareIntensity.value,
                        _lensFlareAspScreenSpace.warpedFlareIntensity.value),
                    new Vector4(
                        _lensFlareAspScreenSpace.vignetteEffect.value,
                        _lensFlareAspScreenSpace.startingPosition.value,
                        _lensFlareAspScreenSpace.scale.value,
                        0), // Free slot, not used
                    new Vector4(
                        _lensFlareAspScreenSpace.samples.value,
                        _lensFlareAspScreenSpace.sampleDimmer.value,
                        _lensFlareAspScreenSpace.chromaticAbberationIntensity.value,
                        0), // No need to pass a chromatic aberration sample count, hardcoded at 3 in shader
                    new Vector4(
                        _lensFlareAspScreenSpace.streaksIntensity.value,
                        _lensFlareAspScreenSpace.streaksLength.value,
                        _lensFlareAspScreenSpace.streaksOrientation.value,
                        _lensFlareAspScreenSpace.streaksThreshold.value),
                    new Vector4(
                        ratio,
                        _lensFlareAspScreenSpace.warpedFlareScale.value.x,
                        _lensFlareAspScreenSpace.warpedFlareScale.value.y,
                        0), // Free slot, not used
                    cmd,
                    _screenSpaceLensFlareResult,
                    false);
 ...
 0; i < UnityEditor.SceneView.sceneViews.Count; i++) // Using a foreach on an ArrayList generates garbage ...
                ...
                cmd.SetGlobalTexture(s_lensFlareScreenSpaceBloomMipTexture, bloomMipTexture);
                cmd.SetGlobalTexture(s_lensFlareScreenSpaceSpectralLut, spectralLut);

                // Setting parameters of the effects
                cmd.SetGlobalVector(s_lensFlareScreenSpaceParams1, parameters1);
                cmd.SetGlobalVector(s_lensFlareScreenSpaceParams2, parameters2);
                cmd.SetGlobalVector(s_lensFlareScreenSpaceParams3, parameters3);
                cmd.SetGlobalVector(s_lensFlareScreenSpaceParams4, parameters4);
                cmd.SetGlobalVector(s_lensFlareScreenSpaceParams5, parameters5);
                cmd.SetGlobalColor(s_lensFlareScreenSpaceTintColor, tintColor);
                ...
                if (parameters4.x > 0)
                {
                    ...
                    {
                        even = (i % 2 == 0);
                        cmd.SetGlobalInt(s_lensFlareScreenSpaceMipLevel, i);
                        cmd.SetGlobalTexture(s_lensFlareScreenSpaceStreakTex, even ? streakTextureTmp.Identifier() : streakTextureTmp2.Identifier());
                    ...
                    {
                        even = (i % 2 == 0);
                        cmd.SetGlobalInt(s_lensFlareScreenSpaceMipLevel, (i - startIndex));
                        cmd.SetGlobalTexture(s_lensFlareScreenSpaceStreakTex, even ? streakTextureTmp.Identifier() : streakTextureTmp2.Identifier());
                        UnityEngine.Rendering.CoreUtils.SetRenderTarget(cmd, even ? streakTextureTmp2.Identifier() : streakTextureTmp.Identifier());
                    ...
                    cmd.SetGlobalTexture(s_lensFlareScreenSpaceStreakTex, even ? streakTextureTmp2.Identifier() : streakTextureTmp.Identifier());
                }
                ...
                cmd.SetGlobalTexture(s_lensFlareScreenSpaceResultTexture, result.Identifier());
    ...
    public class ASPScreenSpaceLensFlareFeature : ScriptableRendererFeature
    {
        ...
        public RenderPassEvent InjectionPoint = RenderPassEvent.AfterRenderingPostProcessing;
        private Material _material;
        private ASPScreenSpaceLensFlarePass _aspScreenSpaceLensFlare;

        /// <inheritdoc/>
        public override void Create()
        {
            _aspScreenSpaceLensFlare = new ASPScreenSpaceLensFlarePass(name);
        }
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            ...
            _aspScreenSpaceLensFlare.renderPassEvent = (RenderPassEvent)InjectionPoint;
            _aspScreenSpaceLensFlare.ConfigureInput(ScriptableRenderPassInput.None);
            _aspScreenSpaceLensFlare.SetupMembers(_material);

            renderer.EnqueuePass(_aspScreenSpaceLensFlare);
        }

        /// <inheritdoc/>
        protected override void Dispose(bool disposing)
        {
            _aspScreenSpaceLensFlare.Dispose();
        }
        ...
        public class ASPScreenSpaceLensFlarePass : ScriptableRenderPass
        {
            private Material _lensFlareSSMaterial;
            private RTHandle _copiedColor;
            private RTHandle _streakTmpTexture;
            private RTHandle _streakTmpTexture2;
            private RTHandle _screenSpaceLensFlareResult;
            private ASP.ASPScreenSpaceLensFlare _lensFlareAspScreenSpace;
            private int _writeToBloomPass;
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                _lensFlareAspScreenSpace = VolumeManager.instance.stack.GetComponent<ASP.ASPScreenSpaceLensFlare>();
            ...
            {
                _copiedColor?.Release();
                _streakTmpTexture?.Release();
                _streakTmpTexture2?.Release();
                _screenSpaceLensFlareResult?.Release();
            }
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                using (new ProfilingScope(cmd, new ProfilingSampler("Post Process Screen Space Lens Flare")))
                {
                    ...
                    DrawTriangle(cmd, _lensFlareSSMaterial, _writeToBloomPass);
                    CoreUtils.SetRenderTarget(cmd, cameraData.renderer.cameraColorTargetHandle);
                    Blitter.BlitCameraTexture(cmd, _copiedColor, cameraData.renderer.cameraColorTargetHandle);
                ...
                RenderTextureDescriptor m_Descriptor)
            {
                ...
                if (_lensFlareAspScreenSpace.IsStreaksActive())
                {
                    RenderingUtils.ReAllocateIfNeeded(ref _streakTmpTexture, desc, FilterMode.Bilinear,
                        TextureWrapMode.Clamp, name: "_StreakTmpTexture");
                    RenderingUtils.ReAllocateIfNeeded(ref _streakTmpTexture2, desc, FilterMode.Bilinear,
                        TextureWrapMode.Clamp, name: "_StreakTmpTexture2");
                }

                RenderingUtils.ReAllocateIfNeeded(ref _screenSpaceLensFlareResult, desc, FilterMode.Bilinear,
                    TextureWrapMode.Clamp, name: "_ScreenSpaceLensFlareResult");
                ...
                DoLensFlareScreenSpaceCommon(
                    ...
                    _screenSpaceLensFlareResult,
                    false);
                cmd.SetGlobalTexture(s_bloomTextureId, originalBloomTexture);
            }
                ...
                bool debugView)
            {
                ...
                if (parameters4.x > 0)
                {
                    ...
                    for (int i = 0; i < maxLevelDownsample; i++)
                    {
                        even = (i % 2 == 0);
                        cmd.SetGlobalInt(s_lensFlareScreenSpaceMipLevel, i);
                        cmd.SetGlobalTexture(s_lensFlareScreenSpaceStreakTex,
                    ...
                    cmd.SetGlobalTexture(s_lensFlareScreenSpaceStreakTex, even ? streakTextureTmp2 : streakTextureTmp);
                }
                ...
                // Final pass, we add the result of the previous pass to the Original Bloom Texture.
                cmd.SetGlobalTexture(s_lensFlareScreenSpaceResultTexture, result);
                //UnityEngine.Rendering.CoreUtils.SetRenderTarget(cmd, originalBloomTexture);
                //DrawQuad(cmd, lensFlareShader, writeToBloomPass);
            }
        }
