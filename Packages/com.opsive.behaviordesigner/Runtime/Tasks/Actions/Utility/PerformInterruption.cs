...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Actions
{
    ...
    public class PerformInterruption : ECSActionTask<PerformInterruptionTaskSystem, PerformInterruptionComponent, PerformInterruptionFlag>
    {
        ...
        public override void ClearBufferElement(World world, Entity entity)
        {
            if (world.EntityManager.HasBuffer<PerformInterruptionComponent>(entity)) {
                var performInterruptionComponents = world.EntityManager.GetBuffer<PerformInterruptionComponent>(entity);
                for (int i = 0; i < performInterruptionComponents.Length; ++i) {
                    DisposeBlobAssetReference(performInterruptionComponents[i].InterruptIndicies);
                }
            }
    ...
    public struct PerformInterruptionComponent : IBufferElementData
    {
        ...
        [SerializeField]
#endif
        public BlobAssetReference<IndiciesBlob> InterruptIndicies;
