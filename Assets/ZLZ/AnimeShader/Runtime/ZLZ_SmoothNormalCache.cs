// Assets/ZLZ/AnimeShader/Runtime/ZLZ_SmoothNormalCache.cs
...
namespace ZLZ.AnimeShader
{
    ...
    {
        // Resolved at use, not baked in : customers who bought before the folder
        // restructure still have the package at its old path (see ZLZ_AnimePaths).
        static string PATH => ZLZ_AnimePaths.Folder("Data/ZLZ_SmoothNormalCache.asset");
        ...
        {
            var cache = AssetDatabase.LoadAssetAtPath<ZLZ_SmoothNormalCache>(PATH);
            if (cache != null) return cache;

            // This one asset holds every baked smooth normal in the project. Looking
            // it up by a fixed path alone means a single folder move - ours during a
            // restructure, or the customer tidying their project - silently orphans
            // the lot, and the next bake starts from an empty file with no error to
            // explain where the old data went. The type search costs nothing on the
            // normal path because it only runs once the direct load has failed.
            foreach (var guid in AssetDatabase.FindAssets("t:ZLZ_SmoothNormalCache"))
            {
                var found = AssetDatabase.LoadAssetAtPath<ZLZ_SmoothNormalCache>(
        ...
        public static ZLZ_SmoothNormalCache GetOrCreate()
        {
            ...
            ZLZ_AnimePaths.EnsureFolder("Data");
