Shader "ZLZ/AnimeToon/Character"
{
    Properties
    {
        ...
        // UV Set : which mesh UV channel feeds the material textures (MainTex / Normal / Dissolve /
        // masks). VRoid stores user-edited outfit colours on UV1, so VRM characters need UV1 to
        // show the right clothes. The Face Shadow UV always reads UV0 (it has its own offset /
        // scale rig tuned to the face layout). A mesh without the chosen channel reads (0,0).
        [Enum(UV0,0,UV1,1)] _UVSet ("UV Set", Float) = 0
        ...
        // Ambient Fallback : with NO Directional Light in the scene, synthesise a main light from
        // the ambient probe so Base Color / Rim / Hair Highlight / Specular / Face Shadow / Outline
        // keep working. Scenes that have a Directional Light are not affected at all.
        // 0 = off (classic behaviour : those features go dark without a sun).
        _AmbientFallback ("Ambient Fallback", Range(0, 2)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Forward+ : URP turns _ADDITIONAL_LIGHTS OFF on that path and raises _FORWARD_PLUS
            // instead, so without this variant every additional light silently disappears (the
            // per-object light list the classic loop reads does not exist there). Same pragma as
            // URP's own Lit.shader ; Core.hlsl re-defines _ADDITIONAL_LIGHTS inside the
            // _FORWARD_PLUS variant, and ZLZ_Lighting switches to the clustered loop.
            ...
            #include "../../../Common/Shaders/ZLZ_Lighting.hlsl"
            // Frozen-contract check : whichever ZLZ package the customer imports LAST overwrites
            // the shared Common folder, and an older package silently downgrades ZLZ_Lighting.
            // This pass calls the version-6 API (Forward+ ScreenUV overloads, Ambient Fallback,
            // and ZLZ_BakedMainLightOcclusion) ; failing here with a sentence beats failing below
            // with a cryptic "undeclared identifier".
            #if ZLZ_LIGHTING_VERSION < 6
                #error ZLZ_Lighting.hlsl is older than this shader needs (version 6+, Forward+, Ambient Fallback and baked probe occlusion). Re-import the newest ZLZ package so its ZLZ/Common folder wins.
            #endif
            ...
            struct Attributes
            {
                ...
                float2 uv : TEXCOORD0;
                float2 uv1 : TEXCOORD1;   // UV Set : alternate material UV channel (VRoid outfits)
            ...
            Varyings vert(Attributes v)
            {
                ...
                // UVs
                // UV Set : every material texture follows the dropdown ; the Face Shadow UV below
                // stays on v.uv (UV0) because its offset / scale rig is tuned to the face layout.
                float2 baseUV   = lerp(v.uv, v.uv1, saturate((float)_UVSet));
                o.uv            = TRANSFORM_TEX(baseUV, _MainTex);
                o.uvNormal      = TRANSFORM_TEX(baseUV, _MetalNormalMap);
                o.uvDissolve    = TRANSFORM_TEX(baseUV, _Texture2DDissolve);
                ...
                #if defined(_SWITCH_TO_FACE_MODE_ON)
                    half fa, fs;
                    // The face SDF reads its light direction HERE in the vertex stage, so the
                    // Ambient Fallback must be applied to this read too : with no Directional
                    // Light the fragment lights the body from the ambient-dominant direction,
                    // and the face shadow has to follow the same light or the nose / bang
                    // shadow argues with the body shading. Intensity 0 keeps the raw sun read.
                    ZLZ_ComputeFaceScalars(_AmbientFallback > 0.0
                        ? (float3)ZLZ_EffectiveMainLightDir(ZLZ_AMBIENT_FALLBACK_DEFAULT_DIR)
                        : _MainLightPosition.xyz, fa, fs);
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                // ScreenUV overload (ZLZ_LIGHTING_VERSION 2) : Forward+ stores additional lights
                // per screen tile, so the pixel's screen UV is what finds them there.
                ...
                // Ambient Fallback : with no Directional Light every light-anchored feature below
                // (toon ramp, rim, hair highlight, specular, face SDF) multiplies to black at once.
                // Rewrites lightDir / lightColor / distAtten from the ambient probe ONLY when the
                // main light is absent - a scene with a sun is untouched. See ZLZ_Lighting.hlsl.
                ZLZ_ApplyAmbientFallback((half)_AmbientFallback, ZLZ_AMBIENT_FALLBACK_DEFAULT_DIR,
                    lightDir, lightColor, distAtten);
                ...
                half RecShadow;
                    ...
                    // Baked shadow from static geometry, via light probes. Until this line the
                    // character read the realtime shadow map ONLY, so it stayed fully lit inside a
                    // baked shadow whenever the caster was not in that map - past the shadow
                    // distance, or with Cast Shadows turned off on the static scene, which is the
                    // whole point of Shadowmask mode. Returns 1 where nothing is baked, so
                    // realtime-only scenes are untouched (see ZLZ_Lighting.hlsl).
                    //
                    // Placed AFTER the self-shadow rejection on purpose : that check reasons about
                    // the realtime shadow map's caster depth, and this value has no depth to reason
                    // about - it is a probe constant. Feeding it through the rejection would let the
                    // character reject a shadow the scene really is casting on it.
                    //
                    // Placed BEFORE ZLZ_SmoothShadow so the baked term gets the same toon edge
                    // treatment as the realtime one, instead of appearing as a raw linear darkening.
                    effectiveShadow = min(effectiveShadow, ZLZ_BakedMainLightOcclusion());

                    half smoothShadowAtten = ZLZ_SmoothShadow(effectiveShadow, (half)_ShadowSoftness, (half)_ShadowEdgeSmooth);
        ...
        Pass
        {
            ...
            struct Attributes
            {
                float4 positionOS : POSITION;
                float2 uv         : TEXCOORD0;
                float2 uv1        : TEXCOORD1;   // UV Set : alternate material UV channel
                UNITY_VERTEX_INPUT_INSTANCE_ID
            };
            ...
            Varyings ZLZContactShadowVertex(Attributes i)
            {
                ...
                o.uv          = TRANSFORM_TEX(lerp(i.uv, i.uv1, saturate((float)_UVSet)), _MainTex);
                return o;
            }
        ...
        Pass
        {
            ...
            Varyings DepthOnlyVertex(Attributes i)
            {
                ...
                UNITY_TRANSFER_INSTANCE_ID(i, o);
                o.positionHCS = TransformObjectToHClip(i.positionOS.xyz);
                float2 baseUV = lerp(i.uv, i.uv1, saturate((float)_UVSet));   // UV Set
                o.uv = TRANSFORM_TEX(baseUV, _MainTex);
                o.uvDissolve = TRANSFORM_TEX(baseUV, _Texture2DDissolve);
        ...
        Pass
        {
            ...
            Varyings MotionVectorsVertex(Attributes input)
            {
                ...
                float2 baseUV     = lerp(input.uv, input.uv1, saturate((float)_UVSet));   // UV Set
                output.uv         = TRANSFORM_TEX(baseUV, _MainTex);
                output.uvDissolve = TRANSFORM_TEX(baseUV, _Texture2DDissolve);
        ...
        Pass
        {
            ...
            // Frozen-contract check : whichever ZLZ package the customer imports LAST overwrites
            // the shared Common folder, and an older package silently downgrades ZLZ_Lighting.
            // This shader calls the version-3 API (Forward+ ScreenUV overloads + Ambient
            // Fallback) ; failing here with a sentence beats failing below with a cryptic
            // "no matching overload".
            #if ZLZ_LIGHTING_VERSION < 3
                #error ZLZ_Lighting.hlsl is older than this shader needs (version 3+, Forward+ and Ambient Fallback support). Re-import the newest ZLZ package so its ZLZ/Common folder wins.
            #endif
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                // Outline matches the body : with no Directional Light the body is lit by the
                // Ambient Fallback, and an outline multiplied by the black sun would vanish
                // right next to it. Same fallback, same signal (pure black = no main light).
                {
                    half3 fbDir   = ZLZ_AMBIENT_FALLBACK_DEFAULT_DIR;
                    half  fbAtten = 1.0h;
                    ZLZ_ApplyAmbientFallback((half)_AmbientFallback, ZLZ_AMBIENT_FALLBACK_DEFAULT_DIR,
                        fbDir, lightColor, fbAtten);
                }
