...
public sealed class HelperChainAttackSequenceDef : ScriptableObject
{
    ...
    [Tooltip("Radius around the player's aim target used to find a valid chain attack target for the helper.")]
    [Min(0.1f)] public float aimSearchRadius = 3f;
    [Tooltip("Layers searched for helper chain attack targets.")]
    public LayerMask targetLayers = ~0;
    [Tooltip("Controls whether target search includes trigger colliders.")]
    public QueryTriggerInteraction targetTriggerInteraction = QueryTriggerInteraction.Ignore;
    [Tooltip("Require a clear line from the player's aim point to the selected target anchor before starting helper chain attack.")]
    public bool requireAimLineOfSight;
    [Tooltip("Layers that block helper chain target line of sight. Only used when Require Aim Line Of Sight is enabled.")]
    public LayerMask aimObstacleLayers = 0;

    [Header("Teleport")]
    [Tooltip("Hide the helper at the warp cast moment before teleporting to the resolved chain attack pose.")]
    public bool hideHelperAtWarpCastMoment = true;
    [Tooltip("Use the target anchor rotation as the base facing. Disable to use the helper/player fallback rotation as the base.")]
    ...
    [Tooltip("Try orientation candidates and clearance checks before accepting a helper teleport pose.")]
    ...
    [Tooltip("Local center offset for the legacy clearance box, relative to the resolved helper teleport pose.")]
    ...
    [Header("Debug")]
    [Tooltip("Log helper target search, teleport candidate tests, NavMesh sampling, obstacle hits, and rejection reasons.")]
