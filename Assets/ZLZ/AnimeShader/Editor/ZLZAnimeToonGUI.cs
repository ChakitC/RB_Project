...
namespace ZLZ.AnimeShader.Editor
{
    public class ZLZAnimeToonGUI : ShaderGUI
    {
        ...
        MaterialProperty _ambientFallback;
        ...
        void CacheProperties(MaterialProperty[] properties)
        {
            ...
            _uvSet = FindProperty("_UVSet", properties, false);
            ...
            _ambientFallback = FindProperty("_AmbientFallback", properties, false);
        ...
        void DrawRendering(MaterialEditor me)
        {
            ...
            DrawSection(MakePrefsKey(me, "Rendering"), "Rendering", true, () =>
            {
                ...
                if (_uvSet != null) me.ShaderProperty(_uvSet, new GUIContent("UV Set",
                    "Which mesh UV channel feeds the material textures (Main, Normal, Dissolve, masks). " +
                    "VRoid / VRM characters store user-edited outfit colors on UV1 - pick UV1 there. " +
                    "Face Shadow always reads UV0 (its own offset/scale rig). " +
                    "If the texture stretches from a corner after picking UV1, the mesh has no UV1 channel."));
        ...
        void DrawBaseLighting(MaterialEditor me)
        {
            ...
            DrawSection(MakePrefsKey(me, "BaseCharacterLighting"), "Base Character Lighting", true, () =>
            {
                ...
                if (_ambientFallback != null) me.ShaderProperty(_ambientFallback, new GUIContent("Ambient Fallback",
                    "With NO Directional Light in the scene, synthesizes a main light from the ambient probe so Base Color, " +
                    "Rim Light, Hair Highlight, Specular, Face Shadow and Outline keep working (night / indoor / point-light-only scenes). " +
                    "Scenes that have a Directional Light are not affected at all. 1 = ambient-matched brightness ; 0 = off (classic behaviour)."));
            });
        }
        ...
        void DrawTransparency(MaterialEditor me)
        {
            ...
            {
                DrawAlphaMaskReminder();
                DrawTransparencyMode(me);
        ...
        void DrawOtherProperties(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            foreach (var prop in properties)
            {
                ...
                // Unity 6000.0 exposes these flags through MaterialProperty.flags.
    ...
    {
        // Rendering
        "_RenderQueueSelector","_RenderQueue","_SrcBlend","_DstBlend","_CullMode","_UVSet",
        ...
        // Base lighting
        "_ReceiveShadow","_ShadowSoftness","_ShadowEdgeSmooth","_RejectSelfShadow","_SelfShadowRejectDist","_AdditionalLightIntensity","_AmbientFallback","_UseContactShadow",
        ...
        void DrawFeatureMaskReminder(MaterialProperty channelProp, string featureName)
        {
            ...
            DrawReminderRow(chipText, accent, message);
        ...
        // Chip + one-line message in a box : the shared look of every "where does this read its
        // mask from" reminder (feature masks and the Transparency alpha).
        static void DrawReminderRow(string chipText, Color accent, string message)
        {
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            using (new EditorGUILayout.HorizontalScope())
            {
        ...
        // Transparency's mask is the Main Texture's ALPHA channel (final alpha = Main Tex alpha
        // x AlphaValue), never the Feature Masks. Same chip as the feature reminders, amber when
        // there is no alpha to read : then AlphaValue fades the whole mesh evenly, which is the
        // "my mask does nothing" case customers ask about.
        void DrawAlphaMaskReminder()
        {
            Texture tex    = (_mainTex != null) ? _mainTex.textureValue : null;
            bool   noTex   = (tex == null);
            bool   noAlpha = !noTex && !TextureHasAlpha(tex);

            string chipText = noTex ? "NO TEXTURE" : noAlpha ? "NO ALPHA" : "MAIN TEX · A";
            Color  accent   = (noTex || noAlpha) ? CHIP_ALERT : CHANNEL_CHIP_COLORS[3];
            string message  = noTex   ? "No Main Texture — AlphaValue fades the whole mesh evenly."
                            : noAlpha ? "Main Texture has no alpha channel — AlphaValue fades the whole mesh evenly."
                            :           "Transparency reads its mask from the Main Texture's alpha channel.";

            DrawReminderRow(chipText, accent, message);
        }

        // ── Transparency mode : status chip + Opaque / Transparent buttons ─────
        // The state is read from the material's REAL render values (queue, blend, Alpha Clipping),
        // never from a stored mode flag, so every path that changes them - the Rendering section,
        // Setup Hair Transparent, Undo - shows correctly here. Only two buttons on purpose : cutout
        // stays the Rendering section's own Alpha Clipping toggle, and the buttons never touch
        // AlphaValue, Cutoff, Stencil or Cull.
        const float QUEUE_OPAQUE      = 2000f;
        const float QUEUE_ALPHATEST   = 2450f;
        const float QUEUE_TRANSPARENT = 3000f;
        static readonly Color CHIP_ALPHA_IN_USE = new Color(0.45f, 0.68f, 1.00f); // the alpha is being read

        static bool IsAlphaBlendFactor(float factor)
        {
            int f = Mathf.RoundToInt(factor);
            return f == (int)BlendMode.SrcAlpha || f == (int)BlendMode.OneMinusSrcAlpha || f == (int)BlendMode.SrcAlphaSaturate;
        }

        void DrawTransparencyMode(MaterialEditor me)
        {
            if (_renderQueueSelector == null || _srcBlend == null || _dstBlend == null) return;

            bool mixed = _renderQueueSelector.hasMixedValue || _srcBlend.hasMixedValue || _dstBlend.hasMixedValue
                      || (_alphaClipping != null && _alphaClipping.hasMixedValue);

            bool blendAlpha = IsAlphaBlendFactor(_srcBlend.floatValue) || IsAlphaBlendFactor(_dstBlend.floatValue);
            bool queueTrans = _renderQueueSelector.floatValue >= QUEUE_TRANSPARENT - 0.5f;
            bool clip       = _alphaClipping != null && _alphaClipping.floatValue > 0.5f;
            bool faded      = _alphaValue != null && _alphaValue.floatValue < 0.999f;

            string chipText, message;
            Color  accent;
            if (mixed)
            { chipText = "MIXED";         accent = CHIP_NEUTRAL;      message = "The selected materials use different render modes."; }
            else if (blendAlpha && !queueTrans)
            { chipText = "WRONG QUEUE";   accent = CHIP_ALERT;        message = "Blends with alpha but draws in the Opaque queue — objects behind it can vanish."; }
            else if (!blendAlpha && queueTrans)
            { chipText = "ALPHA IGNORED"; accent = CHIP_ALERT;        message = "Transparent queue, but Blend is One / Zero — the alpha does nothing."; }
            else if (blendAlpha)
            { chipText = "TRANSPARENT";   accent = CHIP_ALPHA_IN_USE; message = "See-through amount = Main Texture alpha × AlphaValue."; }
            else if (clip)
            { chipText = "CUTOUT";        accent = CHIP_ALPHA_IN_USE; message = "Alpha Clipping cuts away pixels whose alpha is below the Cutoff."; }
            else if (faded)
            { chipText = "OPAQUE";        accent = CHIP_ALERT;        message = "AlphaValue has no effect while the material is Opaque."; }
            else
            { chipText = "OPAQUE";        accent = CHIP_NEUTRAL;      message = "Alpha is not used — the mesh draws solid."; }

            DrawReminderRow(chipText, accent, message);

            // A button lights up only when the material already IS that mode ; a mismatched or
            // mixed setup lights neither, so one press is always the way back to a clean state.
            bool isOpaque      = !mixed && !blendAlpha && !queueTrans;
            bool isTransparent = !mixed &&  blendAlpha &&  queueTrans;

            EditorGUILayout.Space(2);
            using (new EditorGUILayout.HorizontalScope())
            {
                bool opaque = GUILayout.Toggle(isOpaque, "Opaque", EditorStyles.miniButtonLeft, GUILayout.Height(22));
                bool transp = GUILayout.Toggle(isTransparent, "Transparent", EditorStyles.miniButtonRight, GUILayout.Height(22));
                if (opaque && !isOpaque) ApplyTransparencyMode(me, false);
                else if (transp && !isTransparent) ApplyTransparencyMode(me, true);
            }
            EditorGUILayout.Space(4);
        }

        // Transparent : queue 3000 + SrcAlpha / OneMinusSrcAlpha. ZWrite is left as it is - Setup
        //               Hair Transparent keeps it On on purpose (no see-through to the inner faces).
        // Opaque      : queue 2000 (2450 while Alpha Clipping is on) + One / Zero + ZWrite On.
        void ApplyTransparencyMode(MaterialEditor me, bool transparent)
        {
            me.RegisterPropertyChangeUndo(transparent ? "Transparency Mode : Transparent" : "Transparency Mode : Opaque");

            if (transparent)
            {
                _renderQueueSelector.floatValue = QUEUE_TRANSPARENT;
            ...
            {
                bool clip = _alphaClipping != null && _alphaClipping.floatValue > 0.5f;
                _renderQueueSelector.floatValue = clip ? QUEUE_ALPHATEST : QUEUE_OPAQUE;
                _srcBlend.floatValue = (float)BlendMode.One;
                _dstBlend.floatValue = (float)BlendMode.Zero;
                if (_zWrite != null) _zWrite.floatValue = 1f;
            }

            _renderSettingsDirty = true; // OnGUI syncs material.renderQueue + the RenderType tag
            GUI.changed = true;
        }

        // Whether the texture carries an alpha channel the shader can read. The importer is
        // asked first : a compressed format name (DXT1 / BC7 report "RGBA") says nothing about
        // whether the source image had alpha. Textures without an importer fall back to the format.
        static bool TextureHasAlpha(Texture tex)
        {
            ...
            if (importer != null)
            {
                ...
                if (importer.alphaSource == TextureImporterAlphaSource.FromInput) return importer.DoesSourceTextureHaveAlpha();
                return true; // FromGrayScale
            }

            return UnityEngine.Experimental.Rendering.GraphicsFormatUtility.HasAlphaChannel(tex.graphicsFormat);
        }
