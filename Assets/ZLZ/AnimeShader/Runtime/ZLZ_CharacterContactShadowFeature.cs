...
namespace ZLZ.AnimeShader
{
    ...
    public class ZLZ_CharacterContactShadowFeature : ScriptableRendererFeature
    {
        ...
        static readonly int ID_ParamsGlobal = Shader.PropertyToID("_ZLZContactShadowParams");
        ...
        public override void Create()
        {
            ...
            // Switching the feature off stops the pass, but it does NOT unbind what the
            // pass published last frame. The character shader keeps sampling a depth
            // texture that nothing updates any more, so the contact shadow stays on
            // screen frozen in place instead of disappearing.
            //
            // URP routes OnValidate straight into Create(), so this runs the moment the
            // checkbox is unticked - in the renderer inspector or in the ZLZ Hub - and
            // again on every domain reload. Zeroing strength (w) is enough: the receiver
            // returns 1.0 (fully lit) regardless of what the stale texture holds.
            if (!isActive) ClearGlobals();
        }

        static void ClearGlobals() => Shader.SetGlobalVector(ID_ParamsGlobal, Vector4.zero);
        ...
        {
            // Same reason as Create(): removing the feature outright must not leave the
            // last published parameters bound behind it.
            ClearGlobals();
            _pass?.Dispose();
        }
