...
namespace Opsive.BehaviorDesigner.Editor.Controls.NodeViews
{
    ...
    public class TaskNodeViewControl : NodeViewBase
    {
        ...
        protected virtual TaskStatus UpdateNodeInternal()
        {
            ...
            if (m_GraphWindow.AttachedToGraph != null) {
                ...
                if (injectedReferences != null) {
                    var expectedReferenceNodeIndex = m_LogicNode != null && m_LogicNode.NodeLookupIndex >= 0 ? m_LogicNode.NodeLookupIndex : -1;
                    for (int i = 0; i < injectedReferences.Length; ++i) {
                        var injectedReference = injectedReferences[i];
                        var referenceNodeIndex = GetInjectedReferenceNodeIndex(injectedReference);
                        if (!IsMatchingInjectedReference(injectedReference, referenceNodeIndex, expectedReferenceNodeIndex)) {
                            continue;
                        }
                        if (m_BehaviorTree.LogicNodes == null || referenceNodeIndex >= m_BehaviorTree.LogicNodes.Length) {
                            continue;
                        }

                        // A reference can map to multiple injected roots. Pick an active/queued node within its injected range.
                        var selectedRuntimeIndex = m_BehaviorTree.LogicNodes[referenceNodeIndex].RuntimeIndex;
                        var hasRunningStatus = false;
                        var rangeEnd = Mathf.Min(m_BehaviorTree.LogicNodes.Length, referenceNodeIndex + Mathf.Max(1, injectedReference.NodeCount));
                        for (int j = referenceNodeIndex; j < rangeEnd; ++j) {
            ...
            if (taskStatus == TaskStatus.Running || taskStatus == TaskStatus.Queued) {
                var nodeProperties = m_LogicNode.NodeProperties as LogicNodeProperties;
                if (nodeProperties != null && m_LastActiveFrame <= Time.frameCount - 1) {
        ...
        /// <summary>
        /// Gets the injected reference node index.
        /// </summary>
        /// <param name="reference">The injected graph reference.</param>
        /// <returns>The injected reference node index.</returns>
        private int GetInjectedReferenceNodeIndex(InjectedGraphReference reference)
        {
            return reference.RuntimeNodeIndex != ushort.MaxValue ? reference.RuntimeNodeIndex : reference.NodeIndex;
        }

        /// <summary>
        /// Returns true if the injected reference matches the displayed reference node.
        /// </summary>
        /// <param name="reference">The injected graph reference.</param>
        /// <param name="referenceNodeIndex">The reference node index.</param>
        /// <param name="expectedReferenceNodeIndex">The expected displayed reference node index.</param>
        /// <returns>True if the reference matches.</returns>
        private bool IsMatchingInjectedReference(InjectedGraphReference reference, int referenceNodeIndex, int expectedReferenceNodeIndex)
        {
            if (expectedReferenceNodeIndex >= 0) {
                return referenceNodeIndex == expectedReferenceNodeIndex;
            }
            return ReferenceEquals(reference.GraphReference, m_Node) || referenceNodeIndex == m_Node.Index;
        }
