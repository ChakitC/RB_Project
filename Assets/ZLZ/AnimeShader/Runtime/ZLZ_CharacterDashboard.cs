// Assets/ZLZ/AnimeShader/Runtime/ZLZ_CharacterDashboard.cs
using UnityEngine;
...
namespace ZLZ.AnimeShader
{
    ...
    public class ZLZ_CharacterDashboardEditor : UnityEditor.Editor
    {
        ...
        void DrawDitherConfig(ZLZ_CharacterVFX vfx, SerializedObject vfxSo)
        {
            ...
            using (new EditorGUI.DisabledScope(!vfx.Dither.Enabled))
            {
                ...
                using (new EditorGUI.DisabledScope(!vfx.Dither.ReceiveOcclusionFade))
                {
                    ...
                    var manager = Object.FindAnyObjectByType<ZLZ_OcclusionFader>(FindObjectsInactive.Include);
#else
...
#if UNITY_2022_2_OR_NEWER
            var manager = Object.FindAnyObjectByType<ZLZ_DarkenManager>(FindObjectsInactive.Include);
#else
        ...
        // ── VFX Helpers ───────────────────────────────────────────────────
        // Resolved, not hardcoded : the package sits at a different path for customers
        // who bought before the folder restructure (see ZLZ_AnimePaths), and a fixed
        // string would quietly create these assets outside their copy of the package.
        static string FXSettingsFolder      => ZLZ_AnimePaths.Folder("FX_Settings");
        static string UpgradeSettingsPath   => FXSettingsFolder + "/ZLZ_UpgradeSettings.asset";
        static string GetHitSettingsPath    => FXSettingsFolder + "/ZLZ_GetHitSettings.asset";
        static string IndicatorSettingsPath => FXSettingsFolder + "/ZLZ_IndicatorSettings.asset";
        static string DissolveSettingsPath  => FXSettingsFolder + "/ZLZ_DissolveSettings.asset";
        static string DitherSettingsPath    => FXSettingsFolder + "/ZLZ_DitherSettings.asset";
...
#if UNITY_2022_2_OR_NEWER
            var existing = Object.FindAnyObjectByType<ZLZ_OcclusionFader>(FindObjectsInactive.Include);
#else
        ...
        static ZLZ_UpgradeSettings VFX_GetOrCreateUpgradeSettings()
        {
            ...
            if (!AssetDatabase.IsValidFolder(FXSettingsFolder))
                AssetDatabase.CreateFolder(ZLZ_AnimePaths.Root, "FX_Settings");

            var so = ScriptableObject.CreateInstance<ZLZ_UpgradeSettings>();
            AssetDatabase.CreateAsset(so, UpgradeSettingsPath);
            AssetDatabase.SaveAssets();
            Debug.Log($"[ZLZ] Created ZLZ_UpgradeSettings at {UpgradeSettingsPath}");
            return so;
        }
        ...
        static ZLZ_GetHitSettings VFX_GetOrCreateGetHitSettings()
        {
            ...
            AssetDatabase.CreateAsset(so, GetHitSettingsPath);
            AssetDatabase.SaveAssets();
            Debug.Log($"[ZLZ] Created ZLZ_GetHitSettings at {GetHitSettingsPath}");
            return so;
        }
        ...
        static ZLZ_IndicatorSettings VFX_GetOrCreateIndicatorSettings()
        {
            ...
            AssetDatabase.CreateAsset(so, IndicatorSettingsPath);
            AssetDatabase.SaveAssets();
            Debug.Log($"[ZLZ] Created ZLZ_IndicatorSettings at {IndicatorSettingsPath}");
            return so;
        }
        ...
        static ZLZ_DissolveSettings VFX_GetOrCreateDissolveSettings()
        {
            ...
            AssetDatabase.CreateAsset(so, DissolveSettingsPath);
            AssetDatabase.SaveAssets();
            Debug.Log($"[ZLZ] Created ZLZ_DissolveSettings at {DissolveSettingsPath}");
            return so;
        }
        ...
        static ZLZ_DitherSettings VFX_GetOrCreateDitherSettings()
        {
            ...
            AssetDatabase.CreateAsset(so, DitherSettingsPath);
            AssetDatabase.SaveAssets();
            Debug.Log($"[ZLZ] Created ZLZ_DitherSettings at {DitherSettingsPath}");
            return so;
        }
        ...
        static List<MeshBakeInfo> CollectMeshBakeInfo(GameObject root)
        {
            ...
            { Mesh m = GetMesh(r); if (m != null && seen.Add(m.GetStableId())) result.Add(new MeshBakeInfo { mesh=m, isBaked=ZLZ_SmoothNormalBaker.IsMeshBaked(m) }); }
            return result;
        }
        ...
        static List<Mesh> GetAllMeshes(GameObject root)
        {
            ...
            foreach (Renderer r in root.GetComponentsInChildren<Renderer>(true))
            { Mesh m = GetMesh(r); if (m != null && seen.Add(m.GetStableId())) result.Add(m); }
            return result;
        }
        ...
        // There used to be a Window > ZLZ > Install Renderer Features menu item here, calling
        // InstallRendererFeatures() directly. It existed as a manual fallback for the case where
        // AutoInstall below legitimately does nothing - no URP renderer yet, or a stale once-flag
        // from an earlier install on this machine (the flag lives in EditorPrefs, per machine
        // rather than per project, so a new project on the same machine could find itself locked
        // out with no way to ask again).
        //
        // Removed because both of its jobs now have an owner the customer can actually see :
        // ZLZ Hub opens on package import and installs across every URP asset and renderer from
        // its Setup page, and this Dashboard's own Setup buttons cover any one feature after that.
        // A third door into the same code could only disagree with them - it wrote to the single
        // active renderer and reported one log line, so a project with per-quality URP assets came
        // away half configured while looking done.
