...
Shader "Toon/TC_CustomToon_Fade"
{
	Properties
	{
		...
		_FadeBottomY("Fade Bottom World Y", Float) = 1
		_FadeSoftness("Fade Softness", Float) = 2
		...
		[HideInInspector]_BlackCapColor("Black Cap Color", Color) = (0,0,0,1)
		[HideInInspector]_BlackCapStrength("Black Cap Strength", Range(0, 1)) = 0
		[HideInInspector]_BlackCapStartY("Black Cap Start World Y", Float) = 1
		[HideInInspector]_BlackCapSoftness("Black Cap Softness", Range(0.01, 10)) = 0.35
		[HideInInspector]_BlackCapNormalThreshold("Black Cap Normal Threshold", Range(0, 1)) = 0.55
	...
	SubShader
	{
		...
		float ToonFadeDitherThreshold( float2 screenPosition, float ditherScale )
		{
			float2 pixel = floor( screenPosition / max( ditherScale, 0.0001 ) );
			float threshold = frac( 52.9829189 * frac( dot( pixel, float2( 0.06711056, 0.00583715 ) ) ) );
			return threshold * 0.984375 + 0.0078125;
		}

		float ToonHeightFadeVisibility( float3 worldPosition, float fade, float fadeBottomY, float fadeSoftness )
		{
		...
		float ToonHeightDitherFadeClipValue( float3 worldPosition, float4 clipPosition, float fade, float fadeBottomY, float fadeSoftness, float ditherScale )
		{
			float visibility = ToonHeightFadeVisibility( worldPosition, fade, fadeBottomY, fadeSoftness );
		...
		float ToonBlackCapMask( float3 worldPosition, float3 worldNormal, float fade, float fadeBottomY, float fadeSoftness, float strength, float startY, float softness, float normalThreshold )
		{
			float fadeActive = 1.0 - saturate( fade );
			float fadeZone = saturate( ( worldPosition.y - fadeBottomY ) / max( fadeSoftness, 0.0001 ) );
			float capZone = saturate( ( worldPosition.y - startY ) / max( abs( softness ), 0.0001 ) );
			float upMask = saturate( ( dot( normalize( worldNormal ), float3( 0, 1, 0 ) ) - normalThreshold ) / max( 1.0 - normalThreshold, 0.0001 ) );
			return saturate( fadeActive * fadeZone * capZone * upMask * strength );
		}
		...
		Pass
		{
			...
			float _InteriorBlackSoftness;
			float _InteriorBlackFadeBoost;
			float4 _BlackCapColor;
			float _BlackCapStrength;
			float _BlackCapStartY;
			float _BlackCapSoftness;
			float _BlackCapNormalThreshold;
