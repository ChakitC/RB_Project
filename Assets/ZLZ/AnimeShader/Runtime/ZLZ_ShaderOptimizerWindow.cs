    ...
    public class ZLZ_ShaderOptimizerWindow : EditorWindow
    {
        ...
        void DrawBarChart()
        {
            ...
            for (int vi = 0; vi < visibleCount; vi++)
            {
                ...
                if (Event.current.type == EventType.MouseDown && hitRect.Contains(Event.current.mousePosition))
                {
                    ...
                    _selectedMatId = _selectedIndex >= 0 ? _analyses[_selectedIndex].mat.GetStableId() : -1;
        ...
        void DrawFeatureListRow(FeatureResult f, Material mat)
        {
            ...
            if (GUI.Button(badge, f.active ? "ON" : "OFF", badgeS))
            {
                ...
                foreach (var a in _analyses)
                    _foldoutCache[a.mat.GetStableId()] = a.foldout;
                _dirty = true;
            }
        ...
        void DrawTexPlatformPanel(TexInfo t)
        {
            ...
            if (changed)
            {
                ...
                foreach (var a in _analyses) _foldoutCache[a.mat.GetStableId()] = a.foldout;
                _dirty = true;
            }
        ...
        void Analyze()
        {
            ...
            foreach (var old in _analyses)
                _foldoutCache[old.mat.GetStableId()] = old.foldout;
            ...
            foreach (var r in _target.GetComponentsInChildren<Renderer>(true))
            {
                ...
                foreach (var mat in r.sharedMaterials)
                {
                    ...
                    if (!seen.Add(mat.GetStableId())) continue;
                    var a = AnalyzeMat(mat);
                    if (_foldoutCache.TryGetValue(mat.GetStableId(), out bool fold))
                        a.foldout = fold;
                    _analyses.Add(a);
                }
            }

            _selectedIndex = _analyses.FindIndex(a => a.mat.GetStableId() == _selectedMatId);
            _cachedOptTex  = null;
        }
