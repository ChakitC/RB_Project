/// <summary>
/// The one team check for character-vs-character gameplay: barriers, taunt, party effects.
///
/// The rule is stated positively - "are these two known to be on opposing sides?" - rather than as
/// "not friendly". A negated friendliness test treats every unknown or neutral actor as hostile,
/// which would make a barrier swallow projectiles it has no business touching and let a taunt grab
/// actors that were never meant to have a side at all.
/// </summary>
public static class CharacterFactionUtility
{
    ...
    static bool TryGetSide(AITargetIdentity identity, out bool isPlayerSide)
    {
        switch (identity)
        {
            ...
            default:
                // Auto, Generic, Neutral: no side, so no faction interaction.
                isPlayerSide = false;
                return false;
        }
    }
